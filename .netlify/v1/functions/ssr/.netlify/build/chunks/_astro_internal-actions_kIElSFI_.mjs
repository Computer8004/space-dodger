import { c as callSafely, a as ActionError, b as ActionInputError, d as ACTION_QUERY_PARAMS, g as getActionQueryString } from './shared_jlo0LvDr.mjs';
import { A as AstroError, t as ActionCalledFromServerError } from './astro/server_MEB9lWux.mjs';
import { i as isActionAPIContext } from './get-action_iaA_rU3G.mjs';
...
async function handleAction(param, path, context) {
  {
    const { getAction } = await import('./get-action_iaA_rU3G.mjs').then(n => n.a);
...
const server = {
  validateAchievement: defineAction({
    input: objectType({
      score: numberType(),
      tier: enumType(["bronze", "silver", "gold", "platinum"])
    }),
    handler: async (input) => {
  ...
  getEligibleTiers: defineAction({
    ...
    handler: async (input) => {
      const eligible = Object.entries(ACHIEVEMENT_TIERS).filter(([_, tier]) => input.score >= tier.minScore).map(([key, tier]) => ({ key, ...tier }));
